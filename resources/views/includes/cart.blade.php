                ...
                <tbody id="cart_view_load" data-target="{{ route('cart.get.load') }}">
                    ...
                    @foreach ($cart as $key => $item)
                        ...
                        <tr>
                                    ...
                                    <div class="product-info">
                                        ...
                                        @foreach ($item['attribute']['option_name'] as $optionkey => $option_name)
                                            <span><em>{{ $item['attribute']['names'][$optionkey] }}:</em>
                                                {{ $option_name }}
                                                ({{ App\Helpers\PriceHelper::setCurrencyPrice($item['attribute']['option_price'][$optionkey]) }})</span>
                                        @endforeach
                                    </div>
                                </div>
                            </td>
                            <td class="text-center text-lg">
                                {{ App\Helpers\PriceHelper::setCurrencyPrice($item['main_price']) }}</td>

                            <td class="text-center">
                                @if ($item['item_type'] != 'digital')
                                    <div class="qtySelector product-quantity">
                                        <span class="decreaseQtycart cartsubclick" data-id="{{ $key }}"
                                            data-target="{{ App\Helpers\PriceHelper::GetItemId($key) }}"><i
                            ...
                            <td class="text-center text-lg">
                                {{ App\Helpers\PriceHelper::setCurrencyPrice($item['main_price'] * $item['qty']) }}
                            </td>
    ...
    <div class="card-body">
        <div class="shopping-cart-footer">
            ...
            <div class="text-right text-lg column {{ Session::has('coupon') ? '' : 'd-none' }}"><span
                    ...
                    class="text-gray-dark">{{ App\Helpers\PriceHelper::setCurrencyPrice(Session::has('coupon') ? round(Session::get('coupon')['discount'], 2) : 0) }}</span>
            </div>
